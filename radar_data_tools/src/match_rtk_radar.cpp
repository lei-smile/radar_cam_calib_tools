                                      ...
                                      char separator, GeographicLib::LocalCartesian& global_gps_trans)
{
        ...
        while (std::getline(ifile, lineStr))
        {
                ...
                //std::cout<<std::setprecision(15)<<"gps_time:  "<<gps_time<<std::endl;
...
int main()
{
        ...
        std::string rtk_radar_out = calib_path + "matched_rtk_radar_trajectory_all.txt";
        ...
        radar_path = calib_path + radar_path;
        ...
        std::vector<std::vector<MatchedCalibData>> matched_calib_trajectories;     //保存匹配的rtk与radar轨迹，用于标定
        for(int i = 0; i < gps_datas_lines.size(); i++)
        {
            // int i =1;
                std::cout<<"********"<<"gps_line:   "<<i<<"********"<<std::endl;
                int gps_line_len = gps_datas_lines[i].size();
                if(gps_line_len < 20)
                {
                    std::cout<<"gps_line is too short!!!"<<std::endl;
                    continue;
                }
                ...
                bool match_successful = false;           //rtk轨迹是否成功匹配到毫米波轨迹的标志
                if(rtk_radar_ID != -1 && min_avgdis < 10.0)       //若min_avgdis过大，则匹配失败
                {
                        std::vector<MatchedCalibData> matched_calib_trajectory;
                        ...
                        std::string rtk_radar_points = "../result/matched_rtk_radar_trajectory_" + std::to_string(i) + ".txt";
                        ...
                        for(int w = 0; w < radar_trajectorys[rtk_radar_ID].size(); w++)
                        {
                                ...
                                {        
                                        if(fabs(radar_point.time - gps_datas_lines[i][j].time_stamp) <= 0.01)
                                        {
                                            ...
                                            std::cout<<"radar_point.time:  "<<radar_point.time<<std::endl;
                                            std::cout<<"gps_time: "<<gps_datas_lines[i][j].time_stamp<<std::endl;
                                                //printf("find rtk point with radar point in the same time\n");
                                ...
                                if(matched_flag)
                                {
                                        MatchedCalibData match_data;
                                        match_data.r = radar_point.r;
                                        match_data.angle = radar_point.angle;
                                        match_data.lat = rtk_point_matched.lat;
                                        match_data.lon = rtk_point_matched.lon;
                                        match_data.height = rtk_point_matched.height;
                                        matched_calib_trajectory.push_back(match_data);
                        ...
                        matched_calib_trajectories.push_back(matched_calib_trajectory);
                }
                ...
                if(match_successful)
                {       
                        avgdis_str = std::to_string(i) + ":    matched" + "   distance:" + std::to_string(min_avgdis).substr(0,4)
                                                               +"    radar_length:"+ std::to_string(radar_trajectory_length); 
                }
                else
                {
                        avgdis_str = std::to_string(i) + ":    not matched" + "   distance:" + std::to_string(min_avgdis).substr(0,4)
                                                               +"    radar_length:"+ std::to_string(radar_trajectory_length); 
                }
        ...
        //处理 matched_calib_trajectories，
        //因为毫米波雷达在转弯时表现较差，暂时只用直行的轨迹作标定
        std::string calib_file = calib_path + "radar_calibdata.txt";
        std::fstream fout_calibdata(calib_file, std::fstream::out);
        fout_calibdata<<std::fixed<<std::setprecision(7);
        std::cout<<"matched_calib_trajectories.size():  "<<matched_calib_trajectories.size()<<std::endl;
        for(int i = 0; i < matched_calib_trajectories.size(); i++)
        {
            int trajectory_length = matched_calib_trajectories[i].size();
            if(trajectory_length < 10)
            {
                continue;
            }
            double start_angle = matched_calib_trajectories[i][0].angle;
            double mid_angle = matched_calib_trajectories[i][trajectory_length/2].angle;
            double end_angle = matched_calib_trajectories[i][trajectory_length-1].angle;
            // if(fabs(mid_angle - start_angle) < 2 && fabs(end_angle - start_angle) < 2)
            if(fabs(end_angle - start_angle) < 8)
            {
                for(int j = 0 ; j < matched_calib_trajectories[i].size(); j++)
                {
                    fout_calibdata << matched_calib_trajectories[i][j].r<<", "<<matched_calib_trajectories[i][j].angle<<", "
                                                   <<matched_calib_trajectories[i][j].lat<<", "<<matched_calib_trajectories[i][j].lon<<", "
                                                   <<matched_calib_trajectories[i][j].height<<std::endl;
                }
            }
        }
