...
cv::FileStorage selectSettings('./select_info.yaml', cv::FileStorage::READ);
...
void readGPSData(std::string fileName, std::vector<GPSinsData>& gps_datas, char separator)
{
  ...
  double lat0 = 0.0, lon0 = 0.0, height0 = 0.0;
  selectSettings["lat0"] >>lat0;
  selectSettings["lon0"] >> lon0;
  selectSettings["height0"] >>height0;
...
int main(int argc, char** argv)
{
  ...
  //calculate delta_time:  中心点偏移除以平均速度,因为rtk与图像时间戳一致性较好，故不去估计delta_t了
  // double gps_time_sum = gps_data[gps_data.size()-1].time_stamp - gps_data[0].time_stamp;
  // std::cout<<"gps_time_sum: "<<gps_time_sum<<std::endl;

  // double radar_time_sum = radar_data_times_wh03[radar_data_times_wh03.size()-1].time
  //                                                       - radar_data_times_wh03[0].time;
  // std::cout<<"radar_time_sum: "<<radar_time_sum<<std::endl;
  // if(fabs(gps_time_sum - radar_time_sum) > 1.5)
  // {
  //   std::cout<<"The trajectory of gps is not as long as that of radar"<<std::endl;
  //   return 0;
  // }
  // std::vector<double> gps_dist_vec;
  // for(int i = 0; i < gps_data.size()-1; i++)
  // {   
  //   double dist = sqrt( pow(gps_data[i+1].x -gps_data[i].x, 2)
  //                                       + pow(gps_data[i+1].y - gps_data[i].y, 2)  );
  //   gps_dist_vec.push_back(dist);
  // }
  // std::vector<double> radar_dist_vec;
  // for(int i = 0; i < radar_data_times_wh03.size(); i ++)
  // {
  //   std::vector<RadarData> points_radar;
  //   points_radar = radar_data_times_wh03[i].radar_datas;
  //   for(int j = 0; j < points_radar.size(); j++)
  //   {
  //     if(points_radar[j].track_id == radar_selectedid)
  //     {
  //       radar_dist_vec.push_back(points_radar[j].r);
  //     }
  //   }
  ...
  {
    std::vector<RadarData> points_radar;
    points_radar = radar_data_times_wh03[j].radar_datas;
    double radar_time = radar_data_times_wh03[j].time;
    GPSinsData gps_selected_data;
    for(int k = 0; k < gps_data.size(); k++)
    {
      if ((gps_data[k].time_stamp - 0.0) < radar_time + delta_t && (gps_data[k+7].time_stamp + 0.0) > radar_time + delta_t)
      {
          std::cout<<"**********!!!!!!!!!!!!"<<std::endl;
          std::cout<<"radar_time:   "<<radar_time<<std::endl;
          std::cout<<"gps_time:   "<<gps_data[k].time_stamp<<std::endl;
          gps_selected_data = gps_data[k];
          break;
      }
    }
    for(int n = 0; n < points_radar.size(); n++)
    {
      if(points_radar[n].track_id == radar_selectedid)
      {
        file_result <<points_radar[n].r<<", "<<points_radar[n].angle<<", "
                              <<gps_selected_data.lat<<", "<<gps_selected_data.lon<<", "
                              <<gps_selected_data.height<<std::endl;
      }
    }
